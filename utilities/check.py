...
class Params(Schema):
    ...
    discord_id = fields.Int(required=True)
...
class User(Schema):
    ...
    params = fields.Nested(Params(), required=True)
...
if __name__ == '__main__':
    ...
    for file in files.files:
        ...
        if result['params']['state'] != "present" and result['params']['state'] != "absent":
            logger.log.error("Значение поля state у пользователя {0} неверно.".format(result['login']))
            exit(1)
