...
def createUsers(gitUsers, ldapUsers, client):
    for gitUser in gitUsers:
        if gitUser not in ldapUsers and gitUsers[gitUser]['params']['state'] == 'present':
            Log.info("Начался процесс создания пользователя: {}.".format(gitUser))
            try:
                client.user_add(
                ...
                Log.info("Начался процесс добавления пользователя: {} в группы.".format(gitUser))
                ...
                Log.info("Пользователь: {} успешно создан.".format(gitUser))
            except FreeipaExceptions.FreeIPAError as e:
                Log.exception(e)
                sys.exit(1)
...
def enableOrDisableUsers(gitUsers, ldapUsers, client):
    for gitUser in gitUsers:
        if gitUser in ldapUsers and gitUsers[gitUser]['params']['state'] == 'absent' and \
            ...
            try:
                if str(ldapUsers[gitUser]['mail'][0]) != str(gitUsers[gitUser]['login']+"@dclab.local") : # Изменение почты при выключении пользователя
                    client.user_mod(a_uid=gitUsers[gitUser]['login'], o_mail=gitUsers[gitUser]['login'])
                client.user_disable(a_uid=gitUsers[gitUser]['login'])
                Log.info("Учетная запись: {} была выключена.".format(gitUsers[gitUser]['login']))
            except FreeipaExceptions.FreeIPAError as e:
                Log.exception(e)
            ...
            try:
                ...
                Log.info("Учетная запись: {} была включена.".format(gitUsers[gitUser]['login']))
            except FreeipaExceptions.FreeIPAError as e:
                Log.exception(e)
    ...
    for gitUser in gitUsers:
        if gitUser in ldapUsers:
            if gitUsers[gitUser]['params']['email'] != str(ldapUsers[gitUser]['mail'][0]):
                try:
                    client.user_mod(a_uid=gitUser, o_mail=gitUsers[gitUser]['params']['email'])
                    if gitUsers[gitUser]['params']['state'] == 'absent':
                        client.user_disable(a_uid=gitUsers[gitUser]['login'])
                    Log.info("Email пользователя: {0} был изменен.".format(gitUsers[gitUser]['login']))
                except FreeipaExceptions.FreeIPAError as e:
                    Log.exception(e)
                    sys.exit(1)
            try:
            ...
            if gitUsers[gitUser]['params']['discord_id'] != ldapDiscordId or ldapDiscordId is None:
                try:
                    ...
                    if gitUsers[gitUser]['params']['state'] == 'absent':
                        client.user_disable(a_uid=gitUsers[gitUser]['login'])
                    Log.info("Discord id пользователя: {0} был изменен".format(gitUsers[gitUser]['login']))
                except FreeipaExceptions.FreeIPAError as e:
                    Log.exception(e)
                    sys.exit(1)
